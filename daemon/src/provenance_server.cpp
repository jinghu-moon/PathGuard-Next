...
ProvenanceServer::ProvenanceServer(std::string socket_path,
...
bool ProvenanceServer::Start() {
    ...
    if (bind(listen_fd_, reinterpret_cast<sockaddr*>(&address), sizeof(address)) != 0
        || chmod(socket_path_.c_str(), 0660) != 0 || listen(listen_fd_, 16) != 0) {
        close(listen_fd_);
        listen_fd_ = -1;
        std::filesystem::remove(socket_path_, ignored);
        return false;
    ...
    worker_ = std::thread(&ProvenanceServer::Run, this);
    return true;
#endif
}
