...
try {
    ...
    [IO.File]::WriteAllText($rules,
        "format = 1`n[apps.`"com.example.app`"]`nredirect=[`"Source`" -> `"Target`"]`n",
    ...
    $plan = (& $adb shell "$remote/zygisk-policy-probe $remote/run/policy.bin com.example.app Source Target") -join "`n"
    if ($LASTEXITCODE -ne 0 -or $plan -notmatch 'redirect=Source->Target') {
    ...
    & $adb shell "$remote/zygisk-policy-probe $remote/run/policy.bin com.example.app Source Target" | Out-Null
