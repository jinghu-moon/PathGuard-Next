...
int main(void)
{
	struct pathguard_vfs_coverage_status status;
	int fd;
	unsigned int index;

	fd = open("/dev/pathguard_vfs_coverage_probe", O_RDONLY | O_CLOEXEC);
	if (fd < 0) {
		perror("open");
		return 1;
	}
	if (ioctl(fd, PATHGUARD_VFS_COVERAGE_IOC_STATUS, &status) < 0) {
		perror("ioctl");
		close(fd);
		return 1;
	}
