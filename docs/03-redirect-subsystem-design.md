...
同步数据面使用 ADR-0005 定义的双挂载后端。运行时优先选择消费固定 FD 的
`strict_fd`；只有 strict capability 不可用、legacy capability 成功且全局配置显式
允许时，设备才可选择 `legacy_namespace_bind`。legacy 是真实 VFS redirect，但保留无法消除的字符串
mount TOCTOU 风险，产品状态不得与 strict 合并。

后端按整个 `ProcessPlan` 一次性选择，而不是逐条规则降级。被选后端必须支持计划
中的全部同步 action；同一事务不得混用 strict 与 legacy。strict 运行时失败后只按
统一事务协议回滚，不得在同一次启动中自动重试 legacy。

原 Phase R0 的 format、resolver 与 mutation lease 契约已经冻结。进入 R1 双后端
executor 前增加 Phase R0.1，补齐三项安全契约：

- `MS_PRIVATE` 等不可逆传播变更的 `namespace_tainted` 终态和 namespace 级终止；
- 计划级后端选择、action mask 与禁止事务中途 strict -> legacy 重试；
- capability 的端到端 probe、两阶段 topology 和 namespace membership 身份模型。
...
canonical identity root: /data/media/<user>
mount source plane: topology 验证后的真实 storage/FUSE mount
```

`redirect A -> B` 生成的核心操作是：

```text
bind real_storage(B) -> visible(A)
```

不是把 mount 目标改成 `/data/media`。`/data/media` 用于规范身份、目录准备和
topology 关联；实际 source 优先来自当前真实 storage mount 或
`/mnt/user/<user>/emulated/<user>`，以保留 FUSE、scoped storage 和 OEM 语义。
直接使用 `/data/media` 必须经过独立设备矩阵验证。

### 7.2 storage topology probe

topology 使用两阶段验证。daemon 在启动和存储重挂载后构建只读候选 snapshot：
...
companion 在目标 app namespace 中再次验证候选 source plane 的 mount ID、parent
...
可确认、应用代码尚未执行的 disposable app namespace；system process、共享
namespace 或成员关系不明确时保持 `unsupported`。

### 7.4 应用顺序

mount 必须 parent-first，使更具体的子路径覆盖父路径：

1. 读取 mountinfo 判断 `/storage` 是否已经满足隔离要求；只有确实需要时，取得
   mutation lease 后执行 `mount(nullptr, "/storage", ..., MS_REC | MS_PRIVATE, ...)`。
   该传播变更必须记录为不可逆 mutation。
2. 建立真实 storage anchor，避免根隔离后丢失真实内容源。
3. 应用 `isolate root`。
4. 恢复 Android 平台必要子树和用户 `allow` 路径。
5. 应用显式 `redirect`。
6. 将固定的 Root-owned、空、000 `deny_anchor` bind 到 `deny` target。
7. 提交共享状态。

默认使用普通 bind；只有确定必须复制子挂载的根隔离操作才使用 recursive bind。

`deny_anchor` 只承诺禁止遍历、列举和修改目录内容，不保证
`stat(deny_mountpoint)` 失败，也不影响应用已经持有的 FD。产品不得把 deny 描述为
隐藏挂载点元数据；legacy deny 必须在专项真机矩阵通过后单独解锁。

### 7.5 后端接口边界

规则编译、父子覆盖、应用顺序、lease、取消和状态机由统一事务器负责。后端只处理
单个准备完成的 mount step：
...
公共类型必须区分 resolver 固定的 `PinnedIdentity` 与仅供 legacy 使用的
`CanonicalLocator`，不得用同一个 `ResolvedSource`/`ResolvedTarget` 暗示 legacy
最终 mount 也消费固定 FD。`MountError` 至少包含阶段、backend、operation ID、
errno、预期/实际 identity 和 namespace taint 标志。

### 7.6 后端 capability 与选择

capability 必须证明端到端 mount 语义，不能只根据 syscall 是否返回 `ENOSYS`：
...
applying -> cancel_requested -> rollback_complete | namespace_tainted
applying -> rollback_complete | namespace_tainted
```

helper 每个阶段检查取消：readiness 前后、`setns` 前、每项 namespace mutation 前、提交前。每次成功的可逆 mount 都写入栈；任何校验、mount、超时或 CAS 失败都按逆序 `umount2(MNT_DETACH)`。

`pending` 状态超时时，应用可以 CAS 为 `cancel_requested` 并安全 fail-open，因为 helper 尚未取得 namespace mutation lease。helper 必须在第一项 namespace mutation 前 CAS 为 `applying`；`MS_PRIVATE`、source anchor 和正向 bind 都不能发生在 lease 之前。
...
ADR-0003 已采用“保持 fail-open，但 `applying` 后等待回滚完成”的契约。300ms 只是
取得 mutation lease 前的取消上限，不是所有命中应用启动的绝对硬上限。内部 5 秒
readiness 上限只能被取消，不能直接变成应用同步等待预算。

`rollback_complete` 只在全部可逆 mount 已验证清除，且传播状态未发生不可恢复变化
时发布。若事务执行过改变原 peer group 的 `MS_PRIVATE`，后续失败即使 mount 栈已
清空也必须发布 `namespace_tainted`，关闭 helper 持有的 namespace/mount FD，并
使执行 `setns` 的 worker 返回原 namespace 或退出，再终止该 namespace 的全部成员；
不得让目标进程继续 fail-open。

失败模式：

- `failure = open`：默认。仅在 namespace 尚未变更，或全部可逆 mutation 已回滚且传播状态未发生不可恢复变化后继续启动，状态为 `failed`，不安装 provider Hook；`namespace_tainted` 必须终止 namespace 成员。
- `failure = closed`：挂载失败时终止目标进程。只有实现并完成真机测试后才允许编译该值；在此之前编译器直接拒绝。

### 8.1 产品状态

安全等级、执行状态和事务结果是正交维度，不使用单一扁平枚举：
...
`unsupported` 是后端选择结果，`failure = open` 是处理策略；两者不得合并。状态同时
报告 snapshot、plan 和 topology generation。
...
| 应用同步等待目标 | lease 前取消目标 `300ms`；`applying` 后等待 rollback 或 namespace taint 终止 |
...
- plan-wide backend selector：action mask 不完整、legacy 未授权、strict 运行时失败和
  topology generation 变化都不得产生逐规则降级或事务中途 fallback。
- fake backend 在每项 mutation、verify、commit 和 rollback 注入失败，验证 journal、
  结构化 `MountError` 与状态转换。
...
- 分别覆盖 `open_tree_move_mount`、`proc_fd_mount`（source/target 双 FD）、
  `string_bind_mount` 和无可用 attach backend；probe 必须执行真实 attach、验证与回滚。
- strict 与 legacy redirect-only 运行同一套目录语义用例；安全验证与风险状态分别断言。
- deny anchor 覆盖 mountpoint/child 的 stat、open、opendir/readdir、create、unlink、
  rename into/out；不得把 mountpoint metadata 可见误判为 deny 失败。
- daemon 候选 topology 与目标 app namespace 二次验证；存储重挂载后旧
  topology/capability generation 必须失效。
...
- helper 已进入 `applying` 时，验证等待 `rollback_complete`；不得用 `pending` 测试替代。
- `/storage` 已满足传播要求和必须执行 `MS_PRIVATE` 两个档位分别注入失败；后者必须
  进入 `namespace_tainted`、关闭 helper FD、终止全部 namespace member 并重新启动。
- PID 复用、进程退出、namespace inode/member 变化、snapshot/plan/topology generation 变化和 daemon 重启。
- legacy 在 mount syscall 并发执行 source/target rename/swap；成功时必须只有一个
  预期 mountinfo delta，身份不明时必须终止 namespace，禁止盲目回滚路径。
- 回滚前额外叠加或移动 target mount，验证 mount ID 不匹配时不会卸载非 PathGuard mount。
...
- [ ] 在支持设备上立即完成 strict redirect-only 垂直链路，分别验证
  `open_tree_move_mount` 或 `proc_fd_mount`。
...
- ADR 0006 已定义 SAF 系统代写进程的试验性写路径虚拟化；该能力必须通过
  `provider = virtualize` 显式启用，不属于基础 mount redirect 能力。
- v0.1.9 的仅写 `open/openat` 方案因破坏 `enforceTree` 被否决。ADR 0006 改为完整目录
  虚拟化，覆盖 create/query/open/rename/delete/canonical/inotify，并在 MediaProvider
  中闭合异步扫描链路。OEM 变体和 shared UID package attribution 仍是 R5 待办。
...
| SRX | 规范 backing 身份、root isolation、allow restore、alias/shared UID 思路 | 每进程 JSON、marker 轮询、广域 Hook、无回滚 mount |
| MaterialCleaner | `/mnt/user` source plane、setns 后字符串 bind、规则顺序模拟 | 无固定 FD 的 TOCTOU、缺少完整事务回滚 |
