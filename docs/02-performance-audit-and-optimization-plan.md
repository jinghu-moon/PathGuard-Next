...
`postAppSpecialize` 在共享状态字上执行 futex 等待，应用侧硬上限为 300ms；超时通过原子 CAS 把状态从 `pending` 改为 `cancel_requested`。helper 在 readiness、每条规则前和最终提交前检查取消；挂载成功只有在 CAS 提交 `complete` 后才保留，否则仍在目标 namespace 中回滚。marker、`fsync`、继承到应用域的跨 SELinux socket 和 25ms 文件轮询路径均已删除。

其中 `setns` 和 mount 是功能必要成本；readiness 退避参数已由真机 P50/P95 数据验证。300ms 是开发期首版 fail-open 上限，不代表最终设备预算。

### 2.3 MediaStore Binder Hook

Hook 位于 `BinderProxy.transactNative`，只在显式 `media_compat=query_filter` 且 mount 成功后安装。Binder descriptor 对 provider binder 做一次性缓存；provider query 先解析 AttributionSource 和 Uri，确认 authority 后才解析 projection、Bundle 和两个 Binder。
...
架构文档将 MediaStore 兼容定位为可选 compat 后端。当前实现已将 Hook 安装门槛收敛为显式 `media_compat=query_filter`，并且只有 mount 成功后才在 `postAppSpecialize` 安装，解决了原实现的两个问题：
...
默认值为 `off`。只有用户明确选择 `query_filter` 且 mount 成功时才安装 Hook；纯 deny/redirect 应用只执行 mount，挂载完成后卸载模块。mount 失败时不安装 Hook，保证 `fail-open` 不残留媒体查询过滤。该字段已经纳入规则模型和 `policy.bin` format v3。
...
3. 将 companion 等待失败的应用侧硬上限收紧到数百毫秒量级，在上限内 fail-open 并记录 `failed`/`stale` 状态。（已完成 300ms 首版上限）
4. 让超时请求具备取消或终止 helper 的协议，避免应用已经放行后 companion 仍在后台改变 namespace。（已完成共享状态 CAS、helper 检查和 mount 回滚；450ms 人工延迟真机验收通过）
...
- policy format v3、共享字段定义和 hash 排序索引。
...
450ms mount 延迟注入覆盖了“第一条 mount 已完成”的超时场景：应用在 300.07ms fail-open，helper 返回 `ECANCELED(125)`、`committed=0`、`rollback_us=506`，目标 namespace 在迟到窗口前后均为 0 个 PathGuard mount，失败路径未安装 MediaStore Hook。此前累计 `hook_cpu_us` 包含原始 Binder transact 等待，不能用来证明 Hook 自身性能；计时边界已改为在调用原始 transact 前结束。
...
- companion 超时不会让应用无条件等待 5 秒，当前应用侧硬上限为 300ms。
- fail-open 后不会有未取消的后台 helper 继续修改运行中 namespace。
- 人为注入超过应用侧上限的 companion 延迟，等待应用 fail-open 后记录目标进程 `mountinfo`，在迟到窗口结束后再次检查，确认没有新增 PathGuard mount、helper 已终止且状态为 `failed`；检查必须结合 PID start time，避免 PID 复用造成误判。（已通过）
