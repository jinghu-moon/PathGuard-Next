...
4. 仅在策略显式启用 `media = hide_denied` 时安装 MediaStore Hook。
5. 连接 Root companion，exempt 结果 socket FD，并发送有界挂载计划。
6. 不执行目录扫描，不在应用侧直接修改 mount namespace。
...
1. 通过共享结果页等待 companion；300ms 只约束 helper 取得 mutation lease 前的取消。
2. 超时时若仍为 `pending`，取消后按 `failure = open` 放行；若已为 `applying`，等待 `rollback_complete` 后放行。
3. 关闭结果 FD；没有安装 Hook 时卸载 Zygisk 模块。
4. 已安装 Hook 时保持模块常驻，供 Binder Hook 函数继续执行。

默认且当前唯一可执行的失败策略为 `failure = open`：仅在 namespace 未变更或回滚完成后继续启动，并明确记录为未保护。`failure = closed` 在完成真机测试前由编译器拒绝。
...
2. 在 `preAppSpecialize` 建立并 exempt companion 通道、发送计划；`postAppSpecialize` 有界等待结果、超时取消并按失败策略收尾。
