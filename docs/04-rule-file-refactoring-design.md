...
14. `->` 的最终采用以脱糖器 Phase D0 Go/No-Go 为前提；“能够安全实现”不自动等于“值得承担实现和维护成本”。若 source-region、位置映射或最小职责边界不满足冻结条件，应在进入生产实现前重新比较严格 TOML。
15. Phase D0 同时决定规则编译链的唯一生产语言与 TOML parser。首选候选是“整条纯编译链使用 Rust + `toml_edit`”，备选是 C++ + toml++；不得把 Rust 只插入脱糖器、再把 AST/span 交回 C++，也不得长期维护双实现。
